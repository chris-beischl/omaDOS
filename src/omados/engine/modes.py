...
from .cards import Cards, Suit, OBER, UNTER, SUIT_MASKS, HERZ, ASSE, NO_CARDS

@dataclass
class GameContract:
    spielart: str
    trumpf_cards: Cards
    rufsau_suit: Optional[Suit] = None
    caller_id: int = -1
    
    def __post_init__(self):
        self.rufsau : Cards = SUIT_MASKS[self.rufsau_suit] & ASSE if self.rufsau_suit is not None else NO_CARDS 
...
class GameModeFactory:
    ...
    @staticmethod
    def create_wenz(caller_id: int, suit : Optional[Suit] = None) -> GameContract:
        if suit is not None: 
            return GameContract(spielart="Wenz", trumpf_cards=UNTER | SUIT_MASKS[suit], caller_id=caller_id) 
    ...
    def create_solo(caller_id: int, suit: Suit = Suit.Herz) -> GameContract:
        return GameContract(spielart="Solo", trumpf_cards=OBER | UNTER | SUIT_MASKS[suit], caller_id=caller_id)
