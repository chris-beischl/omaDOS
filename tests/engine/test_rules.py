...
from omados.engine.cards import CARD_TO_INDEX, Cards, SUIT_MASKS, Suit
...
def test_sauspiel_lead_cannot_run_away_if_short(sauspiel_eichel, rufsau_hand_short):
    ...
    """
    legal = get_legal_moves(rufsau_hand_short, lead_card_idx=-1, contract=sauspiel_eichel)
...
def test_sauspiel_follower_davongelaufen(sauspiel_eichel, rufsau_hand):
    ...
    legal = get_legal_moves(hand, lead_card_idx=e7_idx, contract=sauspiel_eichel, ran_away=True)
...
def test_sauspiel_free_cannot_discard_rufsau(sauspiel_eichel, rufsau_hand):
    """If a different suit (Gras) is led and the player is free, they cannot discard the Ace."""
...
def test_sauspiel_free_can_discard_rufsau_if_ran_away(sauspiel_eichel, rufsau_hand):
    ...
    g7_idx = CARD_TO_INDEX['G7']
    legal = get_legal_moves(rufsau_hand, lead_card_idx=g7_idx, contract=sauspiel_eichel, ran_away=True)
